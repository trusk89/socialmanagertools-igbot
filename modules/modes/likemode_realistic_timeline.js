/**
 * MODE: likemode_realistic_location
 * =====================
 * Goto random location from config list, like fast 12 photo and sleep X min in night.
 *
...
class Likemode_realistic_location extends Status {
	constructor() {
		super();
		this.core = core;
		this.LOG_NAME = "likemode_realistic_location";
	...
	/**
	 * Likemode Realistic Location Flow
	 * =====================
	 * Run this flow
	 *
	 */
	async flow() {
		let tag = "likemode_realistic_location::flow()";
		...
		do {
			...
			if (!is_sleep_night_flag_enabled || is_day) {
				...
				if (this.local.cache_locations.length <= 0) {
					this.local.cache_locations = this.core.config.bot_mode_options[this.core.config.bot_mode].instagram_location_gps.slice();
				}
...
module.exports = Likemode_realistic_location;