/**
 * MODE: likemode_superlike_location
 * =====================
 * Goto random location from config list, go to first photo, go to user profile, like fast 3 photo all time in loop.
 *
...
class Likemode_superlike_location extends Status {
	constructor() {
		super();
		this.core = core;
		this.LOG_NAME = "likemode_superlike_location";
		...
		this.local.profile_counter = parseInt(12 / this.core.config.bot_mode_options[this.core.config.bot_mode].superlike_n);
		this.local.like_counter = this.core.config.bot_mode_options[this.core.config.bot_mode].superlike_n;
		this.local.like_counter_max = 12;
	 ...
	 */
	async flow() {
		let tag = "likemode_superlike_location::flow()";
		...
		do {
			...
			if (!is_sleep_night_flag_enabled || is_day) {
				...
				let response_read_photos_list_profile = await this.api.read.photos_list("profile");
				this.local.cache_photos_ids_profile = this.utils.mix_array(response_read_photos_list_profile.photos_ids);

				this.local.like_counter = this.core.config.bot_mode_options[this.core.config.bot_mode].superlike_n;
				this.local.like_counter_max = 12;
				do {
					this.local.current_photo_id_profile = this.local.cache_photos_ids_profile.pop();

					await this.api.goto.photo(this.local.current_photo_id_profile);
				...
				} while (this.local.like_counter >= 1 && this.local.like_counter_max > 1 && this.local.cache_photos_ids_profile.length > 1);

				this.local.profile_counter--;
				this.log.debug(tag, `local.profile_counter: ${this.local.profile_counter}`);

				if (this.local.profile_counter < 1) {
					this.local.profile_counter = parseInt(12 / this.core.config.bot_mode_options[this.core.config.bot_mode].superlike_n);
...
module.exports = Likemode_superlike_location;