...
class Likemode_classic {
        ...
        if (this.status.CURRENT == this.status.OK) {
            try {
                ...
                if (this.status.CURRENT == this.status.ERROR) {
                    ...
                    this.utils.logger(LOG.WARNING, "like", "error bot :( not like photo, now bot sleep 1-2h");
     ...
     */
    async start() {
        this.utils.logger(LOG.INFO, "likemode", "classic");
        ...
        do {
            ...
            if (sec < sec_min && like_status >= 1) {
                this.utils.logger(LOG.INFO, "likemode", "seconds of loop " + sec + "... for miss ban bot wait " + (sec_min - sec) + "-" + (sec_max - sec));
